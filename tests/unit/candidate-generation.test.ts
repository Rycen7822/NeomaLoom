...
import { generateCandidates, precomputeSearchableText } from '../../packages/core/src/locator/candidate-generation.js';
...
async function createLargeCurrentTextDb(projectRoot: string): Promise<void> {
  const spansDir = path.join(projectRoot, '.noemaloom', 'spans');
  await mkdir(spansDir, { recursive: true });
  const db = new DatabaseSync(path.join(spansDir, 'spans.db'));
  const repoPath = 'docs/large-current.md';
  try {
    applySpanMigrations(db);
    db.exec('BEGIN');
    db.prepare(`INSERT INTO repo_files
      (path, absolute_path, role, language, content_hash, size_bytes, modified_at, indexed_at, generated, ignored, metadata_json)
      VALUES (?, ?, 'canonical_api_doc', 'markdown', 'indexed-before-large-file-changed', 300000, 0, 0, 0, 0, '{}')`).run(repoPath, path.join(projectRoot, repoPath));
    db.prepare(`INSERT INTO repo_spans
      (span_id, path, kind, role, label, start_line, end_line, language, heading_path_json, symbol_path_json,
       stable_locator_json, text_hash, indexed_text, summary, metadata_json, source, updated_at)
      VALUES ('large-current-span', ?, 'doc.paragraph', 'canonical_api_doc', 'Large current text', 1, 1, 'markdown', '["Large"]',
       '[]', ?, 'large-indexed-text-hash', 'needleLarge current text', 'needleLarge current text', '{}', 'test', 0)`).run(repoPath, stableLocator(repoPath, 0));
    db.prepare(`INSERT INTO refresh_revisions
      (graph_revision, project_root, target, started_at, finished_at, file_count, span_count, edge_count, warnings_json)
      VALUES ('rev-large-current-text', ?, 'all', 0, 1, 1, 1, 0, '[]')`).run(projectRoot);
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
    throw error;
...
describe('candidate generation', () => {
  it('precomputes searchable row text once for classifier and evidence paths', () => {
    const text = precomputeSearchableText({
      path: 'src/config.ts',
      kind: 'config.key',
      role: 'config_file',
      label: 'ApiClientEndpoint',
      heading_path_json: '["Config"]',
      symbol_path_json: '["ApiClientEndpoint"]',
      indexed_text: 'export const endpoint = process.env.API_URL',
      summary: 'Endpoint configuration',
      metadata_json: '{"env":"API_URL"}'
    });

    expect(text.lower).toContain('src/config.ts');
    expect(text.lower).toContain('endpoint configuration');
    expect(text.original).toContain('ApiClientEndpoint');
    expect(text.symbol).toContain('["ApiClientEndpoint"]');
    expect(text.config).toContain('{"env":"API_URL"}');
  });
  ...
  it('does not read huge current files just to validate locator boundaries', async () => {
    const projectRoot = await mkdtemp(path.join(tmpdir(), 'noemaloom-large-current-text-'));
    await mkdir(path.join(projectRoot, 'docs'), { recursive: true });
    await writeFile(path.join(projectRoot, 'docs/large-current.md'), `needleLarge\n${'x'.repeat(300_000)}\n`, 'utf8');
    await createLargeCurrentTextDb(projectRoot);

    const generated = await generateCandidates({ projectRoot, query: 'needleLarge', targetRoles: ['canonical_api_doc'] });
    const candidate = generated.candidates.find(item => item.spanId === 'large-current-span');

    expect(candidate?.boundary.stale).toBe(false);
    expect(candidate?.boundary.risk).toBe('low');
    expect(candidate?.boundary.warnings).not.toContain('file content hash differs from index');
  });
});
